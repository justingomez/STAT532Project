...
node
#prune
prune<-as.numeric(sample(which(!is.na(eta[,2])),1))
kill<-which(as.numeric(eta[,3])==prune)
if(is.na(eta[kill[1],2])) also<-which(is.na(eta[kill[1],3]))
if(is.na(eta[kill[2],2])) also<-which(is.na(eta[kill[2],3]))
eta[kill,c(2:4)]<-NA
@
